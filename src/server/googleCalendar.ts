...
import { netapp } from "googleapis/build/src/apis/netapp";
...
export async function getCalendarEventTimes(clerkUserId:string, {start,end}:{start:Date; end:Date}){
 ...
 const events =  await google.calendar("v3").events.list({
    ...
    return events.data.items?.map((event)=>{
        ...
        if(event.start?.date != null && event.end?.date ){
           return {
...
async function getOAuthClient(clerkUserId: string){
   const token = await clerkClient.users.getUserOauthAccessToken(clerkUserId,"google")
